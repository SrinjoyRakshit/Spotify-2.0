...
import { pauseMaster, playMaster, playSong } from "../../states/actors/SongActors";
...
const SongBar = () => {
  ...
  const mouseLeave = () => {
    document.querySelector(".active_progress").style.background = 'white'
  }
  ...
  const leaveVolume = () => {
    document.querySelector("#volume").style.background = 'white'
  }
  ...
  const forwardSong = () => {
    ...
    dispatch(playSong(songs[songsIdx+1]))
  }
  ...
  return (
    <div className="fixed px-2 flex items-center justify-between left-0 bottom-0 h-[5rem] w-full bg-black">
      <div className="w-2/12">
        <div className="flex items-center gap-2">
          <img src="/assets/Arijit.jpg" className="h-12" alt="" />
          <div>
          ...
          <FaRegHeart className="ml-3" />
          <MdSmartScreen className="ml-3" />
        </div>
      </div>
      <div className="w-5/12">
        <div className="flex items-center justify-center gap-5 mb-2">
          <FaShuffle />
          <FaBackwardStep onClick={backwardSong} />
          {isPlaying ? (
            <button
              onClick={handleMaster}
              className="flex items-center justify-center p-2 rounded-[50%] bg-white"
            >
          ...
          <FaForwardStep onClick={forwardSong} />
          <RiRepeatLine />
        </div>
        <div className="flex items-center gap-2">
